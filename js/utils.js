  ...
  function () {
    ...
    for (let i = 0; i < len1; i++) {
      str = str.replace(
        ...
        `${StrUtil.TITLE_UPPER_WORDS_PLURAL[i]
          .slice(0, -1)
          .toUpperCase()}${StrUtil.TITLE_UPPER_WORDS_PLURAL[i]
          .slice(-1)
          .toLowerCase()}`
      );
    }
    ...
    str = str
      .split(/([;:?!.])/g)
      .map((pt) =>
        pt.replace(/^(\s*)([^\s])/, (...m) => `${m[1]}${m[2].toUpperCase()}`)
      )
      .join("");
...
String.prototype.toSingle =
  String.prototype.toSingle ||
  function () {
    return this.replace(/i?e?s$/i, "");
  };

String.prototype.toPlural =
  String.prototype.toPlural ||
  function () {
    let plural;
...
String.prototype.countSubstring =
  String.prototype.countSubstring ||
  function (term) {
...
Array.prototype.joinConjunct ||
  Object.defineProperty(Array.prototype, "joinConjunct", {
    enumerable: false,
    writable: true,
...
globalThis.StrUtil = {
  ...
  TITLE_UPPER_WORDS_PLURAL: [
    ...
    "Dcs",
    "D&d",
  ], // (Manually pluralize, to avoid infinite loop)
...
globalThis.CleanUtil = {
  getCleanJson(data, { isMinify = false, isFast = true } = {}) {
    ...
    data = MiscUtil.getWalker().walk(data, {
      string: (str) => CleanUtil.getCleanString(str, { isFast }),
    });
    let str = isMinify
      ? JSON.stringify(data)
...
globalThis.SourceUtil = class {
  ...
  static isKnownSource(source) {
    return (
      SourceUtil.isSiteSource(source) ||
      (typeof PrereleaseUtil !== "undefined" &&
        PrereleaseUtil.hasSourceJson(source)) ||
  ...
  static isPartneredSourceWotc(source) {
    ...
    if (
      ...
      return !!PrereleaseUtil.sourceJsonToSource(source).partnered;
    if (typeof BrewUtil2 !== "undefined" && BrewUtil2.hasSourceJson(source))
      return !!BrewUtil2.sourceJsonToSource(source).partnered;
    return false;
  }
  ...
  static isPrereleaseSource(source) {
    ...
    if (
      ...
      return true;
    return (
      source.startsWith(Parser.SRC_UA_PREFIX) ||
  ...
  static isClassicSource(source) {
    this._CLASSIC_THRESHOLD_TIMESTAMP ||= new Date(
      Parser.sourceJsonToDate(Parser.SRC_XPHB)
    );
    return (
      new Date(Parser.sourceJsonToDate(source)) <
  ...
  static getFilterGroup(source) {
    ...
    if (
      ...
      return SourceUtil.FILTER_GROUP_PRERELEASE;
    if (typeof BrewUtil2 !== "undefined" && BrewUtil2.hasSourceJson(source))
      return SourceUtil.FILTER_GROUP_HOMEBREW;
    if (SourceUtil.isNonstandardSourceWotc(source))
  ...
  static getFilterGroupName(group) {
    switch (group) {
      case SourceUtil.FILTER_GROUP_NON_STANDARD:
        return "Other";
      case SourceUtil.FILTER_GROUP_PRERELEASE:
        return "Prerelease";
      case SourceUtil.FILTER_GROUP_HOMEBREW:
        return "Homebrew";
      case SourceUtil.FILTER_GROUP_PARTNERED:
        return "Partnered";
      case SourceUtil.FILTER_GROUP_STANDARD:
        return null;
      default:
        throw new Error(`Unhandled source filter group "${group}"`);
    }
  }
  ...
  static getAdventureBookSourceHref(source, page) {
    ...
    const meta =
      this._getAdventureBookSourceHref_fromSite({ source }) ||
    ...
    return `${docPage}#${[mappedSource, page ? `page:${page}` : null]
      .filter(Boolean)
      .join(HASH_PART_SEP)}`;
  }
  ...
  static _getAdventureBookSourceHref_fromSite({ source }) {
    ...
    if (Parser.SOURCES_AVAILABLE_DOCS_ADVENTURE[source])
      return {
  ...
  static _getAdventureBookSourceHref_fromPrerelease({ source }) {
    return this._getAdventureBookSourceHref_fromPrereleaseBrew({
      source,
      brewUtil: PrereleaseUtil,
    });
  }
  static _getAdventureBookSourceHref_fromBrew({ source }) {
    return this._getAdventureBookSourceHref_fromPrereleaseBrew({
      source,
      brewUtil: BrewUtil2,
    });
  ...
  static _getAdventureBookSourceHref_fromPrereleaseBrew({ source, brewUtil }) {
    const contentsAdventure = (
      brewUtil.getBrewProcessedFromCache("adventure") || []
    ).filter((ent) => ent.source.toLowerCase() === source);
    const contentsBook = (
      brewUtil.getBrewProcessedFromCache("book") || []
    ).filter((ent) => ent.source.toLowerCase() === source);
    ...
    return {
      docPage: contentsAdventure.length
        ? UrlUtil.PG_ADVENTURE
        : UrlUtil.PG_BOOK,
      mappedSource: Parser.sourceJsonToJson(source),
    };
  }

  static getEntitySource(it) {
...
globalThis.CurrencyUtil = class {
  ...
  static doSimplifyCoins(obj, opts) {
    ...
    const conversionTable =
      opts.currencyConversionTable ||
      Parser.getCurrencyConversionTable(opts.currencyConversionId);
    ...
    for (let i = 0; i < normalized.length - 1; ++i) {
      ...
      const coinRatio =
        normalized[i + 1].normalizedMult / normalized[i].normalizedMult;

      if (obj[coinCur] && Math.abs(obj[coinCur]) >= coinRatio) {
        const nxtVal =
          obj[coinCur] >= 0
            ? Math.floor(obj[coinCur] / coinRatio)
            : Math.ceil(obj[coinCur] / coinRatio);
    ...
    if (opts.originalCurrency) {
      ...
      normalizedHighToLow.forEach((coinMeta, i) => {
        ...
        if (!prevCoinMeta) {
          // Handle the biggest currency, e.g. platinum--never allow it to increase
          if (nxtCoinMeta) {
            const diff = valNew - valOld;
            if (diff > 0) {
              obj[coinMeta.coin] = valOld;
              const coinRatio =
                coinMeta.normalizedMult / nxtCoinMeta.normalizedMult;
              obj[nxtCoinMeta.coin] =
                (obj[nxtCoinMeta.coin] || 0) + diff * coinRatio;
            }
          }
        } else {
          if (nxtCoinMeta) {
            const diffPrevCoin =
              (opts.originalCurrency[prevCoinMeta.coin] || 0) -
              (obj[prevCoinMeta.coin] || 0);
            const coinRatio =
              prevCoinMeta.normalizedMult / coinMeta.normalizedMult;
            const capFromOld =
              valOld + (diffPrevCoin > 0 ? diffPrevCoin * coinRatio : 0);
    ...
    normalized
      .filter(
        (coinMeta) => obj[coinMeta.coin] === 0 || obj[coinMeta.coin] == null
      )
      .forEach((coinMeta) => {
    ...
    if (opts.isPopulateAllValues)
      normalized.forEach(
        (coinMeta) => (obj[coinMeta.coin] = obj[coinMeta.coin] || 0)
      );
  ...
  static getAsCopper(obj) {
    return Parser.FULL_CURRENCY_CONVERSION_TABLE.map(
      (currencyMeta) => (obj[currencyMeta.coin] || 0) * (1 / currencyMeta.mult)
    ).reduce((a, b) => a + b, 0);
  }
  ...
  static getAsSingleCurrency(obj) {
    ...
    const targetDemonination = Parser.FULL_CURRENCY_CONVERSION_TABLE.find(
      (it) => simplified[it.coin]
    );
    ...
    Object.entries(simplified).forEach(([coin, amt]) => {
      const denom = Parser.FULL_CURRENCY_CONVERSION_TABLE.find(
        (it) => it.coin === coin
      );
      out[targetDemonination.coin] =
        (out[targetDemonination.coin] || 0) +
        (amt / denom.mult) * targetDemonination.mult;
    });
  ...
  static getCombinedCurrency(currencyA, currencyB) {
    ...
    [currencyA, currencyB].forEach((currency) => {
      Object.entries(currency).forEach(([coin, cnt]) => {
        ...
        if (isNaN(cnt))
          throw new Error(
            `Unexpected non-numerical value "${JSON.stringify(
              cnt
            )}" for currency key "${coin}"`
          );
...
class TemplateUtil {
  static initJquery() {
    ...
    globalThis.$$ = (parts, ...args) => {
      ...
      const argsNxt = args.map((arg) => {
        if (arg instanceof Array)
          return arg.flatMap((argSub) =>
            argSub instanceof jQuery ? argSub.get() : argSub
          );
    ...
    globalThis.ee = (parts, ...args) => {
      if (parts instanceof Node) {
        return (...passed) => {
          ...
          Array.from(eleTmp.childNodes).forEach((node) =>
      ...
      const raw = parts.reduce((html, p, ix) => {
        ...
        if (args[myIxArg] instanceof Array)
          return `${html}${args[myIxArg]
            .map((arg) => TemplateUtil._ee_handleArg(eles, arg))
            .join("")}${p}`;
        else
          return `${html}${TemplateUtil._ee_handleArg(
            eles,
            args[myIxArg]
          )}${p}`;
      });
      ...
      Array.from(
        eleTmp.querySelectorAll(`[data-r="true"]`)
      ...
      return childNodes.map((childNode) => e_({ ele: childNode }));
    };
  }
...
globalThis.JqueryUtil = {
  ...
  initEnhancements() {
    ...
    $.fn.extend({
      ...
      disableSpellcheck: function () {
        return this.attr("autocomplete", "new-password")
  ...
  addSelectors() {
    ...
    $.expr[":"].textEquals = (el, i, m) =>
      $(el).text().toLowerCase().trim() === m[3].unescapeQuotes();
    ...
    $.expr[":"].containsInsensitive = (el, i, m) => {
      ...
      const textNode = $(el)
        .contents()
        .filter((i, e) => e.nodeType === 3)[0];
      if (!textNode) return false;
      const match = textNode.nodeValue
        .toLowerCase()
        .trim()
        .match(`${searchText.toLowerCase().trim().escapeRegexp()}`);
  ...
  showCopiedEffect($_ele, text = "Copied!", bubble) {
    ...
    if (bubble) {
      animationOptions.left = `${Math.random() > 0.5 ? "-" : "+"}=${~~(
        Math.random() * 17
      )}`;
    }
    ...
    const $dispCopied = $(
      `<div class="clp__disp-copied ve-flex-vh-center py-2 px-4"></div>`
    );
    $dispCopied
      .html(text)
      .css({
      ...
      .animate(animationOptions, {
        ...
        progress: (_, progress) => {
          ...
          if (bubble) {
            ...
            $dispCopied.css(
              "transform",
              `rotate(${seed > 0.5 ? "-" : ""}${seed * 500 * progress}deg)`
            );
          }
        },
      });
    ...
    if (!JqueryUtil._dropdownInit) {
      ...
      document.addEventListener("click", () =>
        [...document.querySelectorAll(`.open`)]
          .filter(
            (ele) =>
              !(ele.className || "").split(" ").includes(`dropdown--navbar`)
          )
  ...
  doToast(options) {
    ...
    if (JqueryUtil._WRP_TOAST == null) {
      JqueryUtil._WRP_TOAST = e_({
        tag: "div",
        clazz:
          "toast__container no-events w-100 ve-overflow-y-hidden ve-flex-col",
      });
    ...
    const eleToast = e_({
      ...
      click: (evt) => {
        ...
        [...JqueryUtil._ACTIVE_TOAST].forEach((toastMeta) =>
          JqueryUtil._doToastCleanup(toastMeta)
        );
      },
    });
  ...
  _doToastCleanup(toastMeta) {
    ...
    JqueryUtil._ACTIVE_TOAST.splice(
      JqueryUtil._ACTIVE_TOAST.indexOf(toastMeta),
      1
    );
    setTimeout(
      () => toastMeta.eleToast.parentElement && toastMeta.eleToast.remove(),
      85
    );
  },
...
if (typeof window !== "undefined")
  window.addEventListener("load", JqueryUtil.initEnhancements);
...
globalThis.ElementUtil = {
  ...
  getOrModify({
    ...
    if (children)
      for (let i = 0, len = children.length; i < len; ++i)
        if (children[i] != null) ele.append(children[i]);
    ...
    ele.disableSpellcheck =
      ele.disableSpellcheck || ElementUtil._disableSpellcheck.bind(ele);
    ele.onn = ele.onn || ElementUtil._onX.bind(ele);
    ele.onClick = ele.onClick || ElementUtil._onX.bind(ele, "click");
    ele.onContextmenu =
      ele.onContextmenu || ElementUtil._onX.bind(ele, "contextmenu");
  ...
  _getOrModify_getEle({ ele, outer, tag, id }) {
    if (ele) return { ele, isSetId: true };
    if (outer)
      return {
        ele: new DOMParser().parseFromString(outer, "text/html").body
          .childNodes[0],
        isSetId: true,
      };
...
globalThis.MiscUtil = class {
  ...
  static copy(
    obj,
    { isSafe = false, isPreserveUndefinedValueKeys = false } = {}
  ) {
  ...
  static async pCopyBlobToClipboard(blob) {
    ...
    try {
      ...
      if (e.message.includes("Document is not focused")) {
        JqueryUtil.doToast({
          type: "danger",
          content: `Please focus the window first!`,
        });
        return false;
      }
      ...
      JqueryUtil.doToast({
        type: "danger",
        content: `Failed to copy! ${VeCt.STR_SEE_CONSOLE}`,
      });
      throw e;
    }
  }
  ...
  static getThenSetCopy(object1, object2, ...path) {
    ...
    return MiscUtil.set(
      object2,
      ...path,
      MiscUtil.copyFast(val, { isSafe: true })
    );
  }
  ...
  static pDelay(msecs, resolveAs) {
    return new Promise((resolve) =>
      setTimeout(() => resolve(resolveAs), msecs)
    );
  }

  static GENERIC_WALKER_ENTRIES_KEY_BLOCKLIST = new Set([
    "caption",
    "type",
    "colLabels",
    "colLabelGroups",
  ...
  static getWalker(opts) {
    ...
    if (opts.isBreakOnReturn && !opts.isNoModification)
      throw new Error(
        `"isBreakOnReturn" may only be used in "isNoModification" mode!`
      );

    const keyBlocklist = opts.keyBlocklist || new Set();

    const getMappedPrimitive = (
      obj,
      primitiveHandlers,
      lastKey,
      stack,
      ...
      switch (to) {
        ...
        case "object": {
          if (obj instanceof Array) {
            ...
            if (opts.isDepthFirst) {
              if (stack) stack.push(obj);
              const out = await obj.pSerialAwaitMap((it) =>
                pFn(it, primitiveHandlers, lastKey, stack)
              );
              if (!opts.isNoModification) obj = out;
              if (stack) stack.pop();
  ...
  static async _getAsyncWalker_pApplyHandlers({
    opts,
    handlers,
    obj,
    lastKey,
...
globalThis.EventUtil = class {
  ...
  static init() {
    ...
    document.addEventListener("keydown", (evt) => {
      switch (evt.key) {
        case "Shift":
          return (EventUtil._isKeydownShift = true);
        case "Control":
          return (EventUtil._isKeydownCtrlMeta = true);
        case "Meta":
          return (EventUtil._isKeydownCtrlMeta = true);
      }
    });
    document.addEventListener("keyup", (evt) => {
      switch (evt.key) {
        case "Shift":
          return (EventUtil._isKeydownShift = false);
        case "Control":
          return (EventUtil._isKeydownCtrlMeta = false);
        case "Meta":
          return (EventUtil._isKeydownCtrlMeta = false);
      }
    });
  ...
  static _onMouseMove_setCssVars() {
    ...
    EventUtil._eleDocRoot =
      EventUtil._eleDocRoot || document.querySelector(":root");

    EventUtil._eleDocRoot.style.setProperty(
      "--mouse-position-x",
      EventUtil._mouseX
    );
    EventUtil._eleDocRoot.style.setProperty(
  ...
  static getClientX(evt) {
    return evt.touches && evt.touches.length
      ? evt.touches[0].clientX
      : evt.clientX;
  }
  static getClientY(evt) {
    return evt.touches && evt.touches.length
  ...
  static isShiftDown() {
    return EventUtil._isKeydownShift;
  }

  static isCtrlMetaDown() {
    return EventUtil._isKeydownCtrlMeta;
  }

  /* -------------------------------------------- */

  static isUsingTouch() {
    return !!EventUtil._isUsingTouch;
  }
...
if (typeof window !== "undefined")
  window.addEventListener("load", EventUtil.init);
...
globalThis.ContextUtil = {
  ...
  Menu: class {
    ...
    _getMenuPosition(evt, axis, { bounds = null, offset = null } = {}) {
      const { fnMenuSize, fnGetEventPos, fnWindowSize, fnScrollDir } =
        axis === "x"
          ? {
              fnMenuSize: "width",
    ...
    closeSubMenus(menuSubExclude = null) {
      this._menusSub
        .filter(
          (menuSub) => menuSubExclude == null || menuSub !== menuSubExclude
        )
  ...
  Action: function (text, fnAction, opts) {
    ...
    this.render = function ({ menu }) {
      ...
      return {
        action: this,
        $eleRow: $$`<div class="ui-ctx__row ve-flex-v-center ${
          this.style || ""
        }">${$btnAction}${$btnActionAlt}</div>`,
        $eleBtn: $btnAction,
      };
    };
    ...
    this._render_$btnAction = function ({ menu }) {
      const $btnAction = $(
        `<div class="w-100 min-w-0 ui-ctx__btn py-1 pl-5 ${
          this.fnActionAlt ? "" : "pr-5"
        }" ${this.isDisabled ? "disabled" : ""} tabindex="0">${this.text}</div>`
      )
        .on("click", async (evt) => {
    ...
    this._render_$btnActionAlt = function ({ menu }) {
      ...
      const $btnActionAlt = $(
        `<div class="ui-ctx__btn ml-1 bl-1 py-1 px-4" ${
          this.isDisabled ? "disabled" : ""
        }>${
          this.textAlt ?? `<span class="glyphicon glyphicon-cog"></span>`
        }</div>`
      )
        .on("click", async (evt) => {
          if (this.isDisabled) return;
  ...
  ActionLink: function (text, fnHref, opts) {
    ...
    this._render_$btnAction = function () {
      this._$btnAction = $(
        `<a href="${this.fnHref()}" class="w-100 min-w-0 ui-ctx__btn py-1 pl-5 ${
          this.fnActionAlt ? "" : "pr-5"
        }" ${this.isDisabled ? "disabled" : ""} tabindex="0">${this.text}</a>`
      );
...
globalThis.UrlUtil = {
  ...
  async pAutoDecodeHash(hash, { page = null } = {}) {
    ...
    if ([UrlUtil.PG_ADVENTURE, UrlUtil.PG_BOOK].includes(page))
      return UrlUtil._pAutoDecodeHashAdventureBookHash(hash, { page });
  ...
  link(href, { isBustCache = false } = {}) {
    if (isBustCache)
      return UrlUtil._link_getWithParam(href, { param: `t=${Date.now()}` });
    return href;
  },
  ...
  categoryToHoverPage(category) {
    return (
      UrlUtil.CAT_TO_HOVER_PAGE[category] || UrlUtil.categoryToPage(category)
    );
  },

  pageToDisplayPage(page) {
    return UrlUtil.PG_TO_NAME[page] || (page || "").replace(/\.html$/, "");
  },
      ...
      lvlFeatures.forEach((lvlFeature) => {
        lvlFeature.forEach((feature, ixFeature) => {
          const subclassFeatureHash = `${UrlUtil.URL_TO_HASH_BUILDER[
            ...
            subclass: sc,
            feature: { ixLevel: feature.level - 1, ixFeature: ixFeature },
          })}`;
  ...
  getClassesPageStatePart(opts) {
    ...
    if (!opts.feature)
      return UrlUtil.packSubHash("state", [
  ...
  _getClassesPageStatePart_feature(feature) {
    return `feature=${UrlUtil.mini.compress(
      `${feature.ixLevel}-${feature.ixFeature}`
    )}`;
  },
};
...
UrlUtil.URL_TO_HASH_GENERIC = (it) =>
  UrlUtil.encodeArrayForHash(it.name, it.source);
...
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_BACKGROUNDS] =
  UrlUtil.URL_TO_HASH_GENERIC;
...
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_CONDITIONS_DISEASES] =
  UrlUtil.URL_TO_HASH_GENERIC;
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_FEATS] = UrlUtil.URL_TO_HASH_GENERIC;
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_OPT_FEATURES] =
  UrlUtil.URL_TO_HASH_GENERIC;
...
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_VARIANTRULES] =
  UrlUtil.URL_TO_HASH_GENERIC;
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_ADVENTURE] = (it) =>
  UrlUtil.encodeForHash(it.id);
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_ADVENTURES] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_ADVENTURE];
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_BOOK] = (it) =>
  UrlUtil.encodeForHash(it.id);
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_BOOKS] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_BOOK];
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_DEITIES] = (it) =>
  UrlUtil.encodeArrayForHash(it.name, it.pantheon, it.source);
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_CULTS_BOONS] =
  UrlUtil.URL_TO_HASH_GENERIC;
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_OBJECTS] = UrlUtil.URL_TO_HASH_GENERIC;
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_TRAPS_HAZARDS] =
  UrlUtil.URL_TO_HASH_GENERIC;
...
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_CHAR_CREATION_OPTIONS] =
  UrlUtil.URL_TO_HASH_GENERIC;
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_RECIPES] = (it) =>
  `${UrlUtil.encodeArrayForHash(it.name, it.source)}${
    it._scaleFactor
      ? `${HASH_PART_SEP}${VeCt.HASH_SCALED}${HASH_SUB_KV_SEP}${it._scaleFactor}`
      : ""
  }`;
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_DECKS] = UrlUtil.URL_TO_HASH_GENERIC;
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_CLASS_SUBCLASS_FEATURES] = (it) =>
  it.__prop === "subclassFeature" || it.subclassSource
    ? UrlUtil.URL_TO_HASH_BUILDER["subclassFeature"](it)
    : UrlUtil.URL_TO_HASH_BUILDER["classFeature"](it);
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_CREATURE_FEATURES] =
  UrlUtil.URL_TO_HASH_GENERIC;
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_VEHICLE_FEATURES] =
  UrlUtil.URL_TO_HASH_GENERIC;
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_OBJECT_FEATURES] =
  UrlUtil.URL_TO_HASH_GENERIC;
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_TRAP_FEATURES] =
  UrlUtil.URL_TO_HASH_GENERIC;
UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_QUICKREF] = ({
  name,
  ixChapter,
  ixHeader,
}) => {
  const hashParts = [
    "bookref-quick",
    ixChapter,
    UrlUtil.encodeForHash(name.toLowerCase()),
  ];
...
UrlUtil.URL_TO_HASH_BUILDER["monster"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_BESTIARY];
UrlUtil.URL_TO_HASH_BUILDER["spell"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_SPELLS];
UrlUtil.URL_TO_HASH_BUILDER["background"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_BACKGROUNDS];
UrlUtil.URL_TO_HASH_BUILDER["item"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_ITEMS];
UrlUtil.URL_TO_HASH_BUILDER["itemGroup"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_ITEMS];
UrlUtil.URL_TO_HASH_BUILDER["baseitem"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_ITEMS];
UrlUtil.URL_TO_HASH_BUILDER["magicvariant"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_ITEMS];
UrlUtil.URL_TO_HASH_BUILDER["class"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_CLASSES];
UrlUtil.URL_TO_HASH_BUILDER["condition"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_CONDITIONS_DISEASES];
UrlUtil.URL_TO_HASH_BUILDER["disease"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_CONDITIONS_DISEASES];
UrlUtil.URL_TO_HASH_BUILDER["status"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_CONDITIONS_DISEASES];
UrlUtil.URL_TO_HASH_BUILDER["feat"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_FEATS];
UrlUtil.URL_TO_HASH_BUILDER["optionalfeature"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_OPT_FEATURES];
UrlUtil.URL_TO_HASH_BUILDER["psionic"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_PSIONICS];
UrlUtil.URL_TO_HASH_BUILDER["race"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_RACES];
UrlUtil.URL_TO_HASH_BUILDER["subrace"] = (it) =>
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_RACES]({
    name: `${it.name} (${it.raceName})`,
    source: it.source,
  });
UrlUtil.URL_TO_HASH_BUILDER["reward"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_REWARDS];
UrlUtil.URL_TO_HASH_BUILDER["variantrule"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_VARIANTRULES];
UrlUtil.URL_TO_HASH_BUILDER["adventure"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_ADVENTURES];
UrlUtil.URL_TO_HASH_BUILDER["adventureData"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_ADVENTURES];
UrlUtil.URL_TO_HASH_BUILDER["book"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_BOOKS];
UrlUtil.URL_TO_HASH_BUILDER["bookData"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_BOOKS];
UrlUtil.URL_TO_HASH_BUILDER["deity"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_DEITIES];
UrlUtil.URL_TO_HASH_BUILDER["cult"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_CULTS_BOONS];
UrlUtil.URL_TO_HASH_BUILDER["boon"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_CULTS_BOONS];
UrlUtil.URL_TO_HASH_BUILDER["object"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_OBJECTS];
UrlUtil.URL_TO_HASH_BUILDER["trap"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_TRAPS_HAZARDS];
UrlUtil.URL_TO_HASH_BUILDER["hazard"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_TRAPS_HAZARDS];
UrlUtil.URL_TO_HASH_BUILDER["table"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_TABLES];
UrlUtil.URL_TO_HASH_BUILDER["tableGroup"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_TABLES];
UrlUtil.URL_TO_HASH_BUILDER["vehicle"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_VEHICLES];
UrlUtil.URL_TO_HASH_BUILDER["vehicleUpgrade"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_VEHICLES];
UrlUtil.URL_TO_HASH_BUILDER["action"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_ACTIONS];
UrlUtil.URL_TO_HASH_BUILDER["language"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_LANGUAGES];
UrlUtil.URL_TO_HASH_BUILDER["charoption"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_CHAR_CREATION_OPTIONS];
UrlUtil.URL_TO_HASH_BUILDER["recipe"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_RECIPES];
UrlUtil.URL_TO_HASH_BUILDER["deck"] =
  UrlUtil.URL_TO_HASH_BUILDER[UrlUtil.PG_DECKS];
...
UrlUtil.URL_TO_HASH_BUILDER["subclass"] = (it) => {
  return Hist.util.getCleanHash(
    ...
    })}${HASH_PART_SEP}${UrlUtil.getClassesPageStatePart({ subclass: it })}`
  );
};
UrlUtil.URL_TO_HASH_BUILDER["classFeature"] = (it) =>
...
UrlUtil.URL_TO_HASH_BUILDER["subclassFeature"] = (it) =>
  UrlUtil.encodeArrayForHash(
    ...
    it.className,
    it.classSource,
    it.subclassShortName,
    it.subclassSource,
    it.level,
    it.source
  );
UrlUtil.URL_TO_HASH_BUILDER["card"] = (it) =>
  UrlUtil.encodeArrayForHash(it.name, it.set, it.source);
...
UrlUtil.URL_TO_HASH_BUILDER["itemProperty"] = (it) =>
  UrlUtil.encodeArrayForHash(it.abbreviation, it.source);
UrlUtil.URL_TO_HASH_BUILDER["itemType"] = (it) =>
  UrlUtil.encodeArrayForHash(it.abbreviation, it.source);
UrlUtil.URL_TO_HASH_BUILDER["itemTypeAdditionalEntries"] =
  UrlUtil.URL_TO_HASH_GENERIC;
...
UrlUtil.URL_TO_HASH_BUILDER["raceFeature"] = (it) =>
  UrlUtil.encodeArrayForHash(it.name, it.raceName, it.raceSource, it.source);
...
Object.keys(UrlUtil.URL_TO_HASH_BUILDER)
  ...
  .forEach(
    (k) =>
      (UrlUtil.URL_TO_HASH_BUILDER[k.toLowerCase()] =
        UrlUtil.URL_TO_HASH_BUILDER[k])
  );
...
Object.keys(UrlUtil.URL_TO_HASH_BUILDER)
  ...
  .forEach(
    (k) =>
      (UrlUtil.URL_TO_HASH_BUILDER[`raw_${k}`] = UrlUtil.URL_TO_HASH_BUILDER[k])
  );
...
Object.keys(UrlUtil.URL_TO_HASH_BUILDER)
  ...
  .forEach((k) => {
    ...
    UrlUtil.URL_TO_HASH_BUILDER[`${k}Template`] =
      UrlUtil.URL_TO_HASH_BUILDER[k];
  });
// endregion
...
UrlUtil.PG_TO_NAME[UrlUtil.PG_CHAR_CREATION_OPTIONS] =
...
UrlUtil.CAT_TO_PAGE[Parser.CAT_ID_CHAR_CREATION_OPTIONS] =
...
UrlUtil.PAGE_TO_PROPS[UrlUtil.PG_ITEMS] = [
  "item",
  "itemGroup",
  "itemType",
  "itemEntry",
    ...
    if (EventUtil.noModifierKeys(e) && typeof d20 === "undefined") {
      if (e.key === "#") {
        ...
        window.prompt(
          ...
          `https://5e.tools/${spl[spl.length - 1]}`
        );
      }
    }
  });
...
globalThis.SortUtil = {
  ...
  ascSortLowerPropNumeric(prop, a, b) {
    a._sortName ||= (a[prop] || "").replace(
      SortUtil._RE_SORT_NUM,
      (...m) => `${m[0].padStart(10, "0")}`
    );
    b._sortName ||= (b[prop] || "").replace(
      SortUtil._RE_SORT_NUM,
      (...m) => `${m[0].padStart(10, "0")}`
    );
  ...
  ascSortDateString(a, b) {
    return SortUtil.ascSortDate(
      new Date(a || "1970-01-01"),
      new Date(b || "1970-01-01")
    );
  },
  ...
  initBtnSortHandlers($wrpBtnsSort, list) {
    ...
    const dispCarets = [...$wrpBtnsSort[0].querySelectorAll(`[data-sort]`)].map(
      (btnSort) => {
        const dispCaret = e_({
          tag: "span",
          clazz: "lst__caret",
  ...
  _initBtnSortHandlers_showCaret({ dispCaret, dispCarets, direction }) {
    ...
    dispCaret
      .addClass("lst__caret--active")
      .toggleClass("lst__caret--reverse", direction === "asc");
  },
  ...
  ascSortAdventure(a, b) {
    return (
      SortUtil.ascSortDateString(b.published, a.published) ||
  ...
  ascSortDeity(a, b) {
    return (
      SortUtil.ascSortLower(a.name, b.name) ||
      SortUtil.ascSortLower(a.source, b.source) ||
      SortUtil.ascSortLower(a.pantheon, b.pantheon)
    );
  },

  ascSortCard(a, b) {
    return (
      SortUtil.ascSortLower(a.set, b.set) ||
      SortUtil.ascSortLower(a.source, b.source) ||
      SortUtil.ascSortLower(a.name, b.name)
    );
  },
  ...
  ascSortEncounter(a, b) {
    return (
      SortUtil.ascSortLower(a.name, b.name) ||
      SortUtil.ascSortLower(a.caption || "", b.caption || "") ||
      SortUtil.ascSort(a.minlvl || 0, b.minlvl || 0) ||
      SortUtil.ascSort(
        a.maxlvl || Number.MAX_SAFE_INTEGER,
  ...
  ascSortItemRarity(a, b) {
    ...
    return (
      (~ixA ? ixA : Number.MAX_SAFE_INTEGER) -
...
class _DataUtilPropConfigMultiSource extends _DataUtilPropConfig {
  ...
  static pLoadIndex() {
    this._P_INDEX =
      this._P_INDEX ||
      DataUtil.loadJSON(
        `${Renderer.get().baseUrl}data/${this._DIR}/${
          this._isFluff ? `fluff-` : ""
        }index.json`
      );
  ...
  static async loadJSON() {
    return this._loadJSON({ isUnmerged: false });
  }
  static async loadUnmergedJSON() {
    return this._loadJSON({ isUnmerged: true });
  }

  static async _loadJSON({ isUnmerged = false } = {}) {
    const index = await this.pLoadIndex();

    const allData = await Object.entries(index).pMap(async ([indexKey, file]) =>
      this._pLoadSourceEntities({ indexKey, isUnmerged, file })
    );
  ...
  static async pLoadSingleSource(source) {
    ...
    return {
      [this._PROP]: await this._pLoadSourceEntities({ indexKey: source, file }),
    };
  }

  static async _pLoadSourceEntities({
    indexKey = null,
    isUnmerged = false,
    file,
  }) {
    ...
    const fnLoad = isUnmerged
      ? DataUtil.loadRawJSON.bind(DataUtil)
      : DataUtil.loadJSON.bind(DataUtil);

    let data = await fnLoad(
      `${Renderer.get().baseUrl}data/${this._DIR}/${file}`
    );
    data = (data[this._PROP] || []).filter(
      MultiSourceUtil.isEntityIndexKeyMatch.bind(this, indexKey, this._PROP)
    );
  ...
  static async _pInitPreData() {
    return (this._P_INIT_PRE_DATA =
      this._P_INIT_PRE_DATA || this._pInitPreData_());
  }
...
class _DataUtilBrewHelper {
  ...
  async pLoadAdventureBookIdsIndex(urlRoot) {
    urlRoot = this._getCleanUrlRoot(urlRoot);
    return DataUtil.loadJSON(
      `${urlRoot}_generated/index-adventure-book-ids.json`
    );
  }
...
globalThis.DataUtil = {
    ...
    for (const k in data) {
      ...
      for (const it of data[k]) {
        ...
        if (it._copy) {
          setTimeout(() => {
            throw new Error(
              `Unresolved "_copy" in entity: ${JSON.stringify(it)}`
            );
          });
        }
      }
  ...
  async _pDoMetaMerge(ident, data, options) {
    if (data._meta) {
        ...
        await Promise.all(
          Object.entries(data._meta.dependencies).map(
            async ([dataProp, sourceIds]) => {
              ...
              const isHasInternalCopies = (
                data._meta.internalCopies || []
              ).includes(dataProp);

              const dependencyData = await Promise.all(
                sourceIds.map((sourceId) =>
                  DataUtil.pLoadByMeta(dataProp, sourceId)
                )
              );
              ...
              const flatDependencyData = dependencyData
                ...
                .filter(Boolean);
              await Promise.all(
                data[dataProp].map((entry) =>
                  DataUtil._pDoMetaMerge_handleCopyProp(
                    dataProp,
                    flatDependencyData,
                    entry,
                    { ...options, isErrorOnMissing: !isHasInternalCopies }
                  )
                )
              );
            }
      ...
      if (data._meta.includes) {
        const includesData = await Promise.all(
          Object.entries(data._meta.includes).map(
            async ([dataProp, sourceIds]) => {
              ...
              const flatIncludesData = includesData
                .map((dd) => dd[dataProp])
                .flat()
                .filter(Boolean);
    ...
    if (data._meta && data._meta.otherSources) {
      await Promise.all(
        Object.entries(data._meta.otherSources).map(
          async ([dataProp, sourceIds]) => {
            const additionalData = await Promise.all(
              Object.entries(sourceIds).map(async ([sourceId, findWith]) => ({
                findWith,
            ...
            additionalData.forEach(({ findWith, dataOther }) => {
              const toAppend = dataOther[dataProp].filter(
                (it) =>
                  it.otherSources &&
  ...
  async pLoadByMeta(prop, source) {
    ...
    switch (prop) {
      ...
      case "subclassFeature": {
        const baseUrlPart = `${Renderer.get().baseUrl}data/${
          DataUtil._MULTI_SOURCE_PROP_TO_DIR[prop]
        }`;
        const index = await DataUtil.loadJSON(
          `${baseUrlPart}/${DataUtil._MULTI_SOURCE_PROP_TO_INDEX_NAME[prop]}`
        );
        if (index[source])
      ...
      default: {
        ...
        if (impl && (impl.getDataUrl || impl.loadJSON)) {
          const data = await (impl.loadJSON
            ? impl.loadJSON()
            : DataUtil.loadJSON(impl.getDataUrl()));
          if (data[prop] && data[prop].some((it) => it.source === source))
            return data;
  ...
  async _pLoadByMeta_pGetPrereleaseBrew(source) {
    ...
    throw new Error(
      `Could not find prerelease/brew URL for source "${source}"`
    );
  },
  ...
  generic: {
    ...
    packUid(ent, tag) {
      ...
      return [
        ent.name,
        (ent.source || "").toLowerCase() === sourceDefault.toLowerCase()
          ? ""
          : ent.source,
      ]
        .join("|")
    ...
    async _pMergeCopy(impl, page, entryList, entry, options) {
      ...
      const it =
        (impl._mergeCache = impl._mergeCache || {})[hash] ||
      ...
      if (!it) {
        if (options.isErrorOnMissing) {
          throw new Error(
            `Could not find "${page}" entity "${entry._copy.name}" ("${entry._copy.source}") to copy in copier "${entry.name}" ("${entry.source}")`
          );
        }
        return;
      }
      ...
      const templateData = entry._copy?._templates
        ? await DataUtil.loadJSON(
            `${Renderer.get().baseUrl}data/bestiary/template.json`
          )
        : null;
      return DataUtil.generic.copyApplier.getCopy(
        impl,
        MiscUtil.copyFast(it),
        entry,
        templateData,
        options
      );
    ...
    copyApplier: class {
      ...
      static _doMod_replaceName({
        ...
        copyTo[prop].forEach((it) => {
          if (it.name)
            it.name = DataUtil.generic._walker_replaceTxt.walk(
              it.name,
              handlers
            );
        });
      ...
      static _doMod_replaceTxt({
        ...
        copyTo[prop].forEach((it) => {
          props.forEach((prop) => {
            if (prop == null) return;
            if (it[prop])
              it[prop] = DataUtil.generic._walker_replaceTxt.walk(
                it[prop],
                handlers
              );
          });
      ...
      static _doMod_appendIfNotExistsArr({
        copyTo,
        copyFrom,
        modInfo,
        msgPtFailed,
      ...
      static _doMod_replaceArr({
        ...
        if (~ixOld) {
          ...
          throw new Error(
            `${msgPtFailed} Could not find "${prop}" item with name "${modInfo.replace}" to replace`
          );
        return false;
      }

      static _doMod_replaceOrAppendArr({
        copyTo,
        copyFrom,
        modInfo,
        msgPtFailed,
      ...
      static _doMod_insertArr({
        ...
        if (!copyTo[prop])
          throw new Error(`${msgPtFailed} Could not find "${prop}" array`);
        copyTo[prop].splice(
          ~modInfo.index ? modInfo.index : copyTo[prop].length,
          0,
          ...modInfo.items
        );
      }

      static _doMod_removeArr({
        copyTo,
        copyFrom,
        modInfo,
        msgPtFailed,
        ...
        if (modInfo.names) {
          ...
          modInfo.names.forEach((nameToRemove) => {
            ...
            if (~ixOld) copyTo[prop].splice(ixOld, 1);
              ...
              if (!modInfo.force)
                throw new Error(
                  `${msgPtFailed} Could not find "${prop}" item with name "${nameToRemove}" to remove`
                );
            }
          });
          ...
          modInfo.items.forEach((itemToRemove) => {
            ...
            if (~ixOld) copyTo[prop].splice(ixOld, 1);
              ...
              throw new Error(
                `${msgPtFailed} Could not find "${prop}" item "${itemToRemove}" to remove`
              );
          });
        } else
          throw new Error(
            `${msgPtFailed} One of "names" or "items" must be provided!`
          );
      }

      static _doMod_renameArr({
        copyTo,
        copyFrom,
        modInfo,
        msgPtFailed,
        ...
        modInfo.renames.forEach((rename) => {
          const ent = copyTo[prop].find((ent) => ent?.name === rename.rename);
          if (!ent) {
            if (isThrow)
              throw new Error(
                `${msgPtFailed} Could not find "${prop}" item with name "${rename.rename}" to rename`
              );
            return;
          }
      ...
      static _doMod_calculateProp({
        ...
        copyTo[prop][modInfo.prop] = eval(
          DataUtil.generic.variableResolver.getCleanMathExpression(toExec)
        );
      }

      static _doMod_scalarAddProp({
        copyTo,
        copyFrom,
        modInfo,
        msgPtFailed,
      ...
      static _doMod_addSaves({ copyTo, copyFrom, modInfo, msgPtFailed }) {
        ...
        Object.entries(modInfo.saves).forEach(([save, mode]) => {
          ...
          const total =
            ...
            Parser.getAbilityModNumber(copyTo[save]);
      ...
      static _doMod_addSpells({ copyTo, copyFrom, modInfo, msgPtFailed }) {
        if (!copyTo.spellcasting)
          throw new Error(
            `${msgPtFailed} Creature did not have a spellcasting property!`
          );
                ...
                if (!spellCategoryOld[kk])
                  ...
                  if (typeof spellCategoryOld[kk] === "object") {
                    if (spellCategoryOld[kk] instanceof Array)
                      ...
                      throw new Error(
                        `${msgPtFailed} Object at key ${kk} not an array!`
                      );
        ...
        ["constant", "will", "ritual"].forEach((prop) => {
          ...
          modInfo[prop].forEach((sp) =>
            (spellcasting[prop] = spellcasting[prop] || []).push(sp)
          );
        });
        ...
        [
          ...
          for (let i = 1; i <= 9; ++i) {
            ...
            if (modInfo[prop][i]) {
              modInfo[prop][i].forEach((sp) =>
                (spellcasting[prop][i] = spellcasting[prop][i] || []).push(sp)
              );
            }

            if (modInfo[prop][e]) {
              modInfo[prop][e].forEach((sp) =>
      ...
      static _doMod_replaceSpells({ copyTo, copyFrom, modInfo, msgPtFailed }) {
        ...
        const handleReplace = (curSpells, replaceMeta, k) => {
          ...
          if (~ix) {
            ...
            throw new Error(
              `${msgPtFailed} Could not find spell "${replaceMeta.replace}" to replace`
            );
        };
      ...
      static _doMod_removeSpells({ copyTo, copyFrom, modInfo, msgPtFailed }) {
        ...
        if (modInfo.spells) {
          ...
          Object.keys(modInfo.spells).forEach((k) => {
            ...
            spells[k].spells = spells[k].spells.filter(
              (it) => !modInfo.spells[k].includes(it)
            );
          });
        }
        ...
        [
          ...
          for (let i = 1; i <= 9; ++i) {
            ...
            if (modInfo[prop][i]) {
              spellcasting[prop][i] = spellcasting[prop][i].filter(
                (it) => !modInfo[prop][i].includes(it)
              );
            }

            if (modInfo[prop][e]) {
              spellcasting[prop][e] = spellcasting[prop][e].filter(
                (it) => !modInfo[prop][e].includes(it)
              );
            }
          }
        });
      ...
      static _doMod_scalarAddHit({
        ...
        const re = /{@hit ([-+]?\d+)}/g;
        copyTo[prop] = this._WALKER.walk(copyTo[prop], {
          string: (str) => {
            return str.replace(
              re,
      ...
      static _doMod_prefixSuffixStringProp({
        copyTo,
        copyFrom,
        modInfo,
        msgPtFailed,
        ...
        MiscUtil.set(
          copyTo,
          ...propPath,
          `${modInfo.prefix || ""}${str}${modInfo.suffix || ""}`
        );
      }

      static _doMod_handleProp({
        copyTo,
        copyFrom,
        modInfos,
        msgPtFailed,
          ...
          if (typeof modInfo === "string") {
            ...
            switch (modInfo.mode) {
              ...
              case "prefixSuffixStringProp":
                return this._doMod_prefixSuffixStringProp({
                  copyTo,
                  copyFrom,
                  modInfo,
                  msgPtFailed,
              ...
              default:
                throw new Error(
                  `${msgPtFailed} Unhandled mode: ${modInfo.mode}`
                );
            }
          }
        });
      ...
      static _doMod({
        ...
        // special case for "no property" modifications, i.e. underscore-key'd
          ...
          this._doMod_handleProp({ copyTo, copyFrom, modInfos, msgPtFailed });
      }
      ...
      static getCopy(
        ...
        templateData,
        {
          isExternalApplicationKeepCopy = false,
        ...
        if (copyMeta._templates?.length) {
          templates = copyMeta._templates
            .map(({ name: templateName, source: templateSource }) => {
              ...
              // TODO(Template) allow templates for other entity types
              const template = templateData.monsterTemplate.find(
                ({ name, source }) =>
                  name.toLowerCase().trim() === templateName &&
                  source.toLowerCase().trim() === templateSource
              );

              if (!template) {
                templateErrors.push(
                  `Could not find traits to apply with name "${templateName}" and source "${templateSource}"`
                );
                return null;
              }
        ...
        if (templateErrors.length)
          throw new Error(`${msgPtFailed} ${templateErrors.join("; ")}`);
        ...
        Object.keys(copyFrom).forEach((k) => {
          ...
          if (copyTo[k] == null) {
            if (
              DataUtil.generic._MERGE_REQUIRES_PRESERVE_BASE[k] ||
              impl?._MERGE_REQUIRES_PRESERVE[k]
            ) {
    ...
    variableResolver: class {
      ...
      static _ResolverTitleShortName = class extends this._ResolverBase {
        ...
        _getResolved({ ent, detail }) {
          return Renderer.monster.getShortName(ent, { isTitleCase: true });
        }
        _getDisplayText({ detail }) {
      ...
      static _ResolverAbilityScore = class extends this._ResolverBase {
        _doVerifyInput({ msgPtFailed, detail }) {
          if (!Parser.ABIL_ABVS.includes(detail))
            throw new Error(
              `${
                msgPtFailed ? `${msgPtFailed} ` : ""
              } Unknown ability score "${detail}"`
            );
        }
      };
      ...
      static _ResolverDc = class extends this._ResolverAbilityScore {
        ...
        _getResolved({ ent, detail }) {
          return (
            8 +
            Parser.getAbilityModNumber(Number(ent[detail])) +
            Parser.crToPb(ent.cr)
          );
        }
        _getDisplayText({ detail }) {
          return `(${detail.toUpperCase()} DC)`;
        }
      };
      ...
      static _ResolverSpellDc = class extends this._ResolverDc {
        mode = "spell_dc";
        _getDisplayText({ detail }) {
          return `(${detail.toUpperCase()} spellcasting DC)`;
        }
      };
      ...
      static _ResolverToHit = class extends this._ResolverAbilityScore {
        ...
        _getDisplayText({ detail }) {
          return `(${detail.toUpperCase()} to-hit)`;
        }
      };
      ...
      static _ResolverDamageMod = class extends this._ResolverAbilityScore {
        ...
        _getDisplayText({ detail }) {
          return `(${detail.toUpperCase()} damage modifier)`;
        }
      };
      ...
      static _ResolverDamageAvg = class extends this._ResolverBase {
        ...
        _getResolved({ ent, detail }) {
          const replaced = detail
            .replace(/\b(?<abil>str|dex|con|int|wis|cha)\b/gi, (...m) =>
              Parser.getAbilityModNumber(Number(ent[m.last().abil]))
            )
            .replace(/\bsize_mult\b/g, () =>
              this._getSizeMult(this._getSize({ ent }))
            );

          // eslint-disable-next-line no-eval
          return Math.floor(
            eval(
              DataUtil.generic.variableResolver.getCleanMathExpression(replaced)
            )
          );
        }
      ...
      static _ResolverSizeMult = class extends this._ResolverBase {
        ...
        _getResolved({ ent, detail }) {
          ...
          return Math.floor(
            eval(
              `${mult} * ${DataUtil.generic.variableResolver.getCleanMathExpression(
                detail
              )}`
            )
          );
      ...
      static resolve({ obj, ent, msgPtFailed = null }) {
        ...
        return DataUtil.generic.variableResolver._WALKER.walk(obj, {
          string: (str) =>
            str.replace(/<\$(?<variable>[^$]+)\$>/g, (...m) => {
      ...
      static getCleanMathExpression(str) {
    ...
    getVersions(
      parent,
      { impl = null, isExternalApplicationIdentityOnly = false } = {}
    ) {
      if (!parent?._versions?.length) return [];

      return parent._versions
        .map((ver) => {
          if (ver._abstract && ver._implementations?.length)
            return DataUtil.generic._getVersions_template({ ver });
        ...
        .filter((ver) => {
          if (!UrlUtil.URL_TO_HASH_BUILDER[ver.__prop])
            throw new Error(`Unhandled version prop "${ver.__prop}"!`);
      ...
      return ver._implementations.map((impl) => {
        ...
        if (cpyImpl._variables) {
          cpyTemplate = MiscUtil.getWalker().walk(cpyTemplate, {
            string: (str) =>
              str.replace(/{{([^}]+)}}/g, (...m) => cpyImpl._variables[m[1]]),
          });
    ...
    _getVersion({
      ...
      ["additionalSources", "otherSources"].forEach((prop) => {
        if (cpyParentEntity[prop]?.length)
          cpyParentEntity[prop] = cpyParentEntity[prop].filter(
            (srcMeta) => srcMeta.source !== version.source
          );
  ...
  monster: class extends _DataUtilPropConfigMultiSource {
    ...
    static getVersions(
      ...
      const additionalVersionData = DataUtil.monster._getAdditionalVersionsData(
        mon
      );
    ...
    static async pPreloadLegendaryGroups() {
      return (DataUtil.monster._pLoadLegendaryGroups ||= (async () => {
    ...
    static getLegendaryGroup(mon) {
      if (
        ...
        return null;
      return DataUtil.monster.legendaryGroupLookup[mon.legendaryGroup.source]?.[
        mon.legendaryGroup.name
      ];
    }
  ...
  spell: class extends _DataUtilPropConfigMultiSource {
    ...
    static async _pInitPreData_() {
      this._SPELL_SOURCE_LOOKUP = await DataUtil.loadRawJSON(
        `${
    ...
    static _mutEntity(sp, { sourcesLookup = null } = {}) {
      ...
      const spSources = (sourcesLookup ?? this._SPELL_SOURCE_LOOKUP)[
    ...
    static _mutSpell_subclass({ sp, spSources }) {
      ...
      Object.entries(spSources.subclass).forEach(
        ([classSource, classNameTo]) => {
                ...
                if (
                  tgt.some(
                    (it) =>
                      ...
                      it.subclass.name === name &&
                      it.subclass.source === source &&
                      ((it.subclass.subSubclass == null &&
                        val.subSubclasses == null) ||
  ...
  itemType: class extends _DataUtilPropConfig {
    ...
    static getUid(
      ent,
      { isMaintainCase = false, isRetainDefault = false } = {}
    ) {
      // <abbreviation>|<source>
  ...
  language: class extends _DataUtilPropConfigSingleSource {
    ...
    static async loadJSON() {
      ...
      const scriptLookup = {};
      (rawData.languageScript || []).forEach((script) =>
        MiscUtil.set(scriptLookup, script.source, script.name, script)
      );
  ...
  race: class extends _DataUtilPropConfigSingleSource {
    ...
    static getPostProcessedSiteJson(
      rawRaceData,
      { isAddBaseRaces = false } = {}
    ) {
      rawRaceData = MiscUtil.copyFast(rawRaceData);
      (rawRaceData.subrace || []).forEach((sr) => {
        const r = rawRaceData.race.find(
          (it) => it.name === sr.raceName && it.source === sr.raceSource
        );
        if (!r)
    ...
    static async loadPrerelease({ isAddBaseRaces = true } = {}) {
      ...
      this._psLoadJson[cacheKey] ||= DataUtil.race._loadPrereleaseBrew({
        isAddBaseRaces,
    ...
    static getPostProcessedPrereleaseBrewJson(
      rawSite,
      brew,
      { isAddBaseRaces = false } = {}
    ) {
      ...
      (brew.subrace || []).forEach((sr) => {
        const rSite = rawSite.race.find(
          (it) => it.name === sr.raceName && it.source === sr.raceSource
        );
        const rBrew = (brew.race || []).find(
          (it) => it.name === sr.raceName && it.source === sr.raceSource
        );
        if (!rSite && !rBrew)
          return JqueryUtil.doToast({
    ...
    static _getApplyUncommonMonstrous(data) {
      ...
      data.raceFluff.forEach((raceFluff) => {
        if (raceFluff.uncommon) {
          ...
          raceFluff.entries.push(
            MiscUtil.copyFast(data.raceFluffMeta.uncommon)
          );
        ...
        if (raceFluff.monstrous) {
          raceFluff.entries = raceFluff.entries || [];
          raceFluff.entries.push(
            MiscUtil.copyFast(data.raceFluffMeta.monstrous)
          );
  ...
  recipe: class extends _DataUtilPropConfigSingleSource {
    ...
    static async loadJSON() {
      return (DataUtil.recipe._pLoadJson =
        DataUtil.recipe._pLoadJson ||
        (async () => {
          return {
  ...
  class: class clazz extends _DataUtilPropConfigCustom {
    ...
    static loadJSON() {
      return (DataUtil.class._pLoadJson =
        DataUtil.class._pLoadJson ||
        (async () => {
          return {
    ...
    static loadRawJSON() {
      return (DataUtil.class._pLoadRawJson =
        ...
        (async () => {
          ...
          return {
            class: MiscUtil.copyFast(
              allData.map((it) => it.class || []).flat()
            ),
            subclass: MiscUtil.copyFast(
              allData.map((it) => it.subclass || []).flat()
            ),
            classFeature: allData.map((it) => it.classFeature || []).flat(),
            subclassFeature: allData
              .map((it) => it.subclassFeature || [])
              .flat(),
          };
        })());
    }
    ...
    static packUidSubclass(ent, { isMaintainCase = false } = {}) {
      ...
      const out = [
        ent.shortName,
        ent.className,
        (ent.classSource || "").toLowerCase() === sourceDefault.toLowerCase()
          ? ""
          : ent.classSource,
        (ent.source || "").toLowerCase() === sourceDefault.toLowerCase()
          ? ""
    ...
    static unpackUidSubclass(uid, opts) {
      opts = opts || {};
      if (opts.isLower) uid = uid.toLowerCase();
      let [shortName, className, classSource, source, displayText] = uid
        .split("|")
        .map((it) => it.trim());
      classSource =
        classSource ||
        (opts.isLower ? Parser.SRC_PHB.toLowerCase() : Parser.SRC_PHB);
      source =
        source ||
        (opts.isLower ? Parser.SRC_PHB.toLowerCase() : Parser.SRC_PHB);
      return {
    ...
    static isValidClassFeatureUid(uid) {
      const { name, className, level } = DataUtil.class.unpackUidClassFeature(
        uid
      );
    ...
    static async pGetSubclassLookup() {
      DataUtil.class._CACHE_SUBCLASS_LOOKUP_PROMISE =
        DataUtil.class._CACHE_SUBCLASS_LOOKUP_PROMISE ||
        (async () => {
          ...
          Object.assign(
            ...
            await DataUtil.loadJSON(
              `${
                Renderer.get().baseUrl
              }data/generated/gendata-subclass-lookup.json`
            )
          );
  ...
  deity: class extends _DataUtilPropConfigSingleSource {
    ...
    static doPostLoad(data) {
      const PRINT_ORDER = data.deity
        ...
        .unique()
        .sort((a, b) =>
          SortUtil.ascSortDateString(
            Parser.sourceJsonToDate(a),
            Parser.sourceJsonToDate(b)
          )
        )
        .reverse();
    ...
    static unpackUidDeity(uid, opts) {
      opts = opts || {};
      if (opts.isLower) uid = uid.toLowerCase();
      let [name, pantheon, source, displayText, ...others] = uid
        .split("|")
        .map((it) => it.trim());
    ...
    static packUidDeity(it) {
      ...
      return [
        it.name,
        (it.pantheon || "").toLowerCase() === "forgotten realms"
          ? ""
          : it.pantheon,
        (it.source || "").toLowerCase() === sourceDefault.toLowerCase()
          ? ""
          : it.source,
      ]
        .join("|")
  ...
  variantrule: class extends _DataUtilPropConfigSingleSource {
    ...
    static async loadJSON() {
      const rawData = await super.loadJSON();
      const rawDataGenerated = await DataUtil.loadJSON(
        `${Renderer.get().baseUrl}data/generated/gendata-variantrules.json`
      );
  ...
  deck: class extends _DataUtilPropConfigCustom {
    ...
    static loadJSON() {
      return (DataUtil.deck._pLoadJson =
        DataUtil.deck._pLoadJson ||
        (async () => {
          return {
    ...
    static loadRawJSON() {
      return (DataUtil.deck._pLoadRawJson =
        DataUtil.deck._pLoadRawJson ||
        DataUtil.loadJSON(`${Renderer.get().baseUrl}data/decks.json`));
    }
    ...
    static unpackUidCard(uid, opts) {
      opts = opts || {};
      if (opts.isLower) uid = uid.toLowerCase();
      let [name, set, source, displayText] = uid
        .split("|")
        .map((it) => it.trim());
      set = set || "none";
      source =
        source ||
        Parser.getTagSource("card", source)[
          opts.isLower ? "toLowerCase" : "toString"
        ]();
      return {
        name,
        set,
  ...
  quickreference: {
    ...
    unpackUid(uid, opts) {
      opts = opts || {};
      if (opts.isLower) uid = uid.toLowerCase();
      let [name, source, ixChapter, ixHeader, displayText] = uid
        .split("|")
        .map((it) => it.trim());
      source =
        source ||
...
globalThis.RollerUtil = {
  ...
  getFullRollCol(lbl) {
    ...
    return `{@dice ${m[1]}${
      m[2]
    }#$prompt_number:title=Enter a ${m[3].trim()}$#|${lbl}}`;
  },
...
function StorageUtilBase() {
  ...
  this.pGetDump = async function () {
    const out = {};
    await Promise.all(
      (await this._pGetPresentKeys()).map(
        async (key) => (out[key] = await this.pGet(key))
      )
    );
    return out;
  };
...
globalThis.CryptUtil = {
  ...
  _rhex: (n) => {
    ...
    for (let j = 0; j < 4; j++) {
      s +=
        CryptUtil._hex_chr[(n >> (j * 8 + 4)) & 0x0f] +
        CryptUtil._hex_chr[(n >> (j * 8)) & 0x0f];
    }
    return s;
  },
...
globalThis.CollectionUtil = {
  ...
  deepEquals(a, b) {
    ...
    if (a && b && typeof a === "object" && typeof b === "object") {
      if (
        CollectionUtil._eq_isPlainObject(a) &&
        CollectionUtil._eq_isPlainObject(b)
      )
  ...
  _eq_areArraysEqual(a, b) {
    ...
    for (let i = 0; i < length; i++)
      if (!CollectionUtil.deepEquals(a[i], b[i])) return false;
    return true;
  },
...
globalThis.Trie = class {
  ...
  findLongestComplete(tokens, node, accum, found) {
    ...
    if (!nodeNxt) {
      if (found.length) {
        const [out] = found
          .sort(SortUtil.ascSortProp.bind(SortUtil, "length"))
          .reverse();
        return out;
      }
      return null;
...
Array.prototype.segregate ||
  Object.defineProperty(Array.prototype, "segregate", {
    ...
    value: function (fnIsValid) {
      return this.reduce(
        ([pass, fail], elem) =>
          fnIsValid(elem) ? [[...pass, elem], fail] : [pass, [...fail, elem]],
        [[], []]
      );
    },
  });
...
Array.prototype.pMap ||
  Object.defineProperty(Array.prototype, "pMap", {
    enumerable: false,
    writable: true,
...
Array.prototype.pSerialAwaitMap ||
  Object.defineProperty(Array.prototype, "pSerialAwaitMap", {
    enumerable: false,
    writable: true,
    value: async function (fnMap) {
      const out = [];
...
Array.prototype.pSerialAwaitFilter ||
  Object.defineProperty(Array.prototype, "pSerialAwaitFilter", {
    enumerable: false,
    writable: true,
...
Array.prototype.pSerialAwaitFind ||
  Object.defineProperty(Array.prototype, "pSerialAwaitFind", {
    enumerable: false,
    writable: true,
    value: async function (fnFind) {
      for (let i = 0, len = this.length; i < len; ++i)
        if (await fnFind(this[i], i, this)) return this[i];
    },
  });

Array.prototype.pSerialAwaitSome ||
  Object.defineProperty(Array.prototype, "pSerialAwaitSome", {
    enumerable: false,
    writable: true,
    value: async function (fnSome) {
      for (let i = 0, len = this.length; i < len; ++i)
        if (await fnSome(this[i], i, this)) return true;
      return false;
    },
  });

Array.prototype.pSerialAwaitFirst ||
  Object.defineProperty(Array.prototype, "pSerialAwaitFirst", {
    enumerable: false,
    writable: true,
...
Array.prototype.pSerialAwaitReduce ||
  Object.defineProperty(Array.prototype, "pSerialAwaitReduce", {
    enumerable: false,
    writable: true,
    value: async function (fnReduce, initialValue) {
      let accumulator = initialValue === undefined ? this[0] : initialValue;
      for (
        let i = initialValue === undefined ? 1 : 0, len = this.length;
        i < len;
        ++i
      ) {
...
Array.prototype.nextWrap ||
  Object.defineProperty(Array.prototype, "nextWrap", {
    enumerable: false,
    writable: true,
    value: function (item) {
...
Array.prototype.prevWrap ||
  Object.defineProperty(Array.prototype, "prevWrap", {
    enumerable: false,
    writable: true,
    value: function (item) {
      const ix = this.indexOf(item);
...
class BookModeViewBase {
  ...
  constructor(opts) {
    ...
    if (this._hashKey && this._stateKey)
      throw new Error(`Only one of "hashKey" and "stateKey" may be specified!`);
  ...
  setStateOpen() {
    ...
    Hist.cleanSetHash(
      `${window.location.hash}${HASH_PART_SEP}${this._hashKey}${HASH_SUB_KV_SEP}true`
    );
  }
  ...
  setStateClosed() {
    ...
    Hist.cleanSetHash(
      window.location.hash.replace(`${this._hashKey}${HASH_SUB_KV_SEP}true`, "")
    );
  }
  ...
  _$getBtnWindowClose() {
    return $(
      `<button class="ve-btn ve-btn-xs ve-btn-danger br-0 bt-0 btl-0 btr-0 bbr-0 bbl-0 h-20p" title="Close"><span class="glyphicon glyphicon-remove"></span></button>`
  ...
  async _$pGetWrpControls({ $wrpContent }) {
    ...
    const lastColumns = StorageUtil.syncGetForPage(
  ...
  _$getBtnNoneVisibleClose() {
    return $(`<button class="ve-btn ve-btn-default">Close</button>`).click(() =>
      this.setStateClosed()
    );
  }
  ...
  async pOpen() {
    ...
    const {
      $wrpContentOuter,
      $wrpContent,
    } = await this._pGetContentElementMetas();
  ...
  async _pGetContentElementMetas() {
    const $wrpContent = $(
      `<div class="bkmv__scroller smooth-scroll ve-overflow-y-auto print__overflow-visible ${
        this._isColumns ? "bkmv__wrp" : "ve-flex-col"
      } w-100 min-h-0"></div>`
    );
    ...
    const {
      cntSelectedEnts,
      isAnyEntityRendered,
    } = await this._pGetRenderContentMeta({ $wrpContent, $wrpContentOuter });
  ...
  async pHandleSub(sub) {
    ...
    if (UrlUtil.unpackSubHash(bookViewHash)[this._hashKey][0] === "true")
      await this.pOpen();
...
globalThis.ExcludeUtil = {
  ...
  async _pInitialise() {
    ...
    try {
      ...
      JqueryUtil.doToast({
        content:
          "Error when loading content blocklist! Purged blocklist data. (See the log for more information.)",
        type: "danger",
      });
      try {
  ...
  _getValidExcludes(excludes) {
    return excludes
      ...
      .filter(
        (it) => it.hash != null && it.category != null && it.source != null
      ); // remove invalid rows
  },
  ...
  async _pExtendList({ toAdd, lockToken }) {
    ...
    MiscUtil.copyFast(toAdd || [])
      .filter(({ hash, category, source }) => {
        ...
        const cacheUid = ExcludeUtil._getCacheUids(
          hash,
          category,
          source,
          true
  ...
  getAllContentBlocklistedHtml() {
    return `<div class="initial-message initial-message--med">(All content <a href="blocklist.html">blocklisted</a>)</div>`;
  },
...
globalThis.ExtensionUtil = class {
  ...
  static async pDoSendStats(evt, ele) {
    const {
      ...
      hash,
      extensionData,
    } = ExtensionUtil._getElementData({ ele });

    if (page && source && hash) {
      let toSend =
        ExtensionUtil._getEmbeddedFromCache(page, source, hash) ||
        (await DataLoader.pCacheAndGet(page, source, hash));

      if (extensionData) {
        switch (page) {
          case UrlUtil.PG_BESTIARY: {
            if (extensionData._scaledCr)
              toSend = await ScaleCreature.scale(
                toSend,
                extensionData._scaledCr
              );
            else if (extensionData._scaledSpellSummonLevel)
              toSend = await ScaleSpellSummonedCreature.scale(
                toSend,
                extensionData._scaledSpellSummonLevel
              );
            else if (extensionData._scaledClassSummonLevel)
              toSend = await ScaleClassSummonedCreature.scale(
                toSend,
                extensionData._scaledClassSummonLevel
              );
          }
        }
      }

      ExtensionUtil._doSend("entity", {
        page,
        entity: toSend,
        isTemp: !!evt.shiftKey,
      });
  ...
  static _getElementData({ ele }) {
    ...
    const extensionData = rawExtensionData
      ? JSON.parse(rawExtensionData)
      : null;
  ...
  static doSendRoll(data) {
    ExtensionUtil._doSend("roll", data);
  }

  static pDoSend({ type, data }) {
    ExtensionUtil._doSend(type, data);
  }
  ...
  static addEmbeddedToCache(page, source, hash, ent) {
    MiscUtil.set(
      ExtensionUtil._CACHE_EMBEDDED_STATS,
      page.toLowerCase(),
      source.toLowerCase(),
      hash.toLowerCase(),
      MiscUtil.copyFast(ent)
    );
  }
...
if (typeof window !== "undefined")
  window.addEventListener("rivet.active", () => (ExtensionUtil.ACTIVE = true));
...
globalThis.VeLock = function ({ name = null, isDbg = false } = {}) {
  ...
  this.pLock = async ({ token = null } = {}) => {
    if (token != null && this._lockMeta?.token === token) {
      ...
      if (this._isDbg)
        console.warn(
          `Lock ${"add".padEnd(this._MSG_PAD_LEN, " ")} "${
            this._name || "(unnamed)"
          }" (now ${this._lockMeta.depth}) at ${this._getCaller()}`
        );
      return token;
    }
    ...
    if (this._isDbg)
      console.warn(
        `Lock ${"acquired".padEnd(this._MSG_PAD_LEN, " ")} "${
          this._name || "(unnamed)"
        }" at ${this._getCaller()}`
      );
  ...
  this.unlock = () => {
    ...
    if (this._lockMeta.depth > 0) {
      ...
      if (this._isDbg)
        console.warn(
          `Lock ${"sub".padEnd(this._MSG_PAD_LEN, " ")} "${
            this._name || "(unnamed)"
          }" (now ${this._lockMeta.depth - 1}) at ${this._getCaller()}`
        );
    ...
    if (this._isDbg)
      console.warn(
        `Lock ${"released".padEnd(this._MSG_PAD_LEN, " ")} "${
          this._name || "(unnamed)"
        }" at ${this._getCaller()}`
      );
...
globalThis.DatetimeUtil = {
  getDateStr({ date, isShort = false, isPad = false } = {}) {
    ...
    return `${isShort ? month.substring(0, 3) : month} ${
      isPad && date.getDate() < 10 ? "\u00A0" : ""
  ...
  getDatetimeStr({ date, isPlainText = false } = {}) {
    ...
    return `${date.getDate()} ${
      !isPlainText ? `<span title="${monthName}">` : ""
    }${monthName.substring(0, 3)}.${
      !isPlainText ? `</span>` : ""
    } ${date.getFullYear()}, ${DatetimeUtil._getPad2(
      date.getHours()
    )}:${DatetimeUtil._getPad2(date.getMinutes())}:${DatetimeUtil._getPad2(
      date.getSeconds()
    )}`;
  },
...
globalThis.EditorUtil = {
  ...
  initEditor(id, additionalOpts = null) {
    ...
    styleSwitcher.addFnOnChange(() =>
      editor.setOptions({ theme: EditorUtil.getTheme() })
    );
    ...
    window.addEventListener("load", () => {
      ...
      [
        ...
        const iv = setInterval(() => {
          ...
          const $tgt = $wrp
            .closest(".cancer__anchor")
            .find(".cancer__disp-cancer");
          if ($tgt.length) {
            anyFound = true;
      ...
      const ivPad = setInterval(() => {
        ...
        $(`.view-col-group--cancer`).append(
          `<div class="w-100 no-shrink" style="height: 110px;"></div>`
        );
      }, 300);
